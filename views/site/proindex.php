...
<div class="col-sm-12">
    ...
    //var_dump($chori_bachat_apangata);die;
        $total_chori=\app\models\ChoriBachat::find()->where(['fk_per_province'=>$user_details->fk_province_id])->count();
        $verified_chhori=\app\models\ChoriBachat::find()->where(['fk_per_province'=>$user_details['fk_province_id']])->andWhere(['or',
            ['status'=>1],
            ['status'=>2],
            ['status'=>3],
            ['status'=>5],
            ...
            $unverified_chhori=\app\models\ChoriBachat::find()->where(['fk_per_province'=>$user_details['fk_province_id']])->andWhere(['or',
            ['status'=>0],
            ['status'=>4]
            ])->count();
        
        $running_chhori=\app\models\ChoriBachat::find()->where(['fk_per_province'=>$user_details->fk_province_id])->andWhere(['status'=>5])->count();
        $withdraw_chhori=\app\models\ChoriBachat::find()->where(['fk_per_province'=>$user_details->fk_province_id])->andWhere(['status'=>6])->count();
        $closed_chhori=\app\models\ChoriBachat::find()->where(['fk_per_province'=>$user_details->fk_province_id])->andWhere(['status'=>7])->count();
        $total_initial=\app\models\PaymentChori::find()->where(['fk_province_id'=>$user_details->fk_province_id])->where(['fk_economic_year'=>$year['economic_year']])->Sum('amount');
        $total_deposit=\app\models\OtherMonthPayment::find()->where(['fk_province_id'=>$user_details->fk_province_id])->where(['fk_year'=>$year['economic_year']])->Sum('amount');
        
        $total_chori=\app\models\ChoriBachat::find()->where(['fk_per_province'=>$user_details['fk_province_id']])->count();
        $total_initial=\app\models\PaymentChori::find()->where(['fk_province_id'=>$user_details['fk_province_id']])->andWhere(['fk_economic_year'=>$year['economic_year']])->Sum('amount');
        $total_deposit=\app\models\OtherMonthPayment::find()->where(['fk_province_id'=>$user_details['fk_province_id']])->andWhere(['fk_year'=>$year['economic_year']])->Sum('amount');
...
<div class="col-sm-4" style="background-color:rgb(76 175 79);height:160px;border-radius:20px;margin-top:2em;margin-bottom:1em;width:24%;margin-left:0.5%;">
        ...
        <!-- <div class="col-sm-12">
        <span class="glyphicon glyphicon-briefcase" style="color:white;font-weight:bold;float:right;font-size:25px;"></span>
        </div> -->
...
<div class="container2" style="margin-bottom:1em;">
    <?php 
    

     $logoimg = Html::img('@web/images/enblem.png', ['id'=>'logo','alt'=>'Image']);
     foreach($district as $dis){ ?>
      <div class="col-sm-3" style="text-align:center;color:blue;">
      <?= Html::a($logoimg,['site/promunicipal','dis_id'=>$dis['id']],['id'=>'districts','class'=>'btn btn-default btn-lg']) ?>
      <br>
      <?= Html::a($dis['district_nepali'],['site/promunicipal','dis_id'=>$dis['id']],['id'=>'name']) ?>
      </div>
  <?php } ?>
    </div>
    </div>

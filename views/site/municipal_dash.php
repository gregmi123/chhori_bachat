...
$municipal_id=\app\models\Municipals::find()->where(['id'=>$mun_id])->one();
$district=\app\models\District::findone(['id'=>$municipal_id['fk_district']]);
...
$this->params['breadcrumbs'][] = ['label' =>$district['district_nepali'],'url'=>['site/promunicipal','dis_id'=>$district['id']]];
}else if($user_details['user_type']==2){
  
}
$this->params['breadcrumbs'][] = ['label' =>$municipal_id['municipal_nepali']];
?>
<style>
 #districts{
...
<div class="col-sm-12">
        ...
        $economic_year=\app\models\Year::find()->where(['id'=>$year['economic_year']])->one();
          ...
          //   ->all();
            ...
            $total_chori=\app\models\ChoriBachat::find()->where(['fk_per_municipal'=>$mun_id])->count();
            $verified_chhori=\app\models\ChoriBachat::find()->where(['fk_per_municipal'=>$mun_id])->andWhere(['or',
            ['status'=>1],
            ['status'=>2],
            ['status'=>3],
            ['status'=>5],
            ...
            $unverified_chhori=\app\models\ChoriBachat::find()->where(['fk_per_municipal'=>$mun_id])->andWhere(['or',
            ['status'=>0],
            ['status'=>4]
            ])->count();
            
            $running_chhori=\app\models\ChoriBachat::find()->where(['fk_per_municipal'=>$mun_id])->andWhere(['status'=>5])->count();
            $withdraw_chhori=\app\models\ChoriBachat::find()->where(['fk_per_municipal'=>$mun_id])->andWhere(['status'=>6])->count();
            $closed_chhori=\app\models\ChoriBachat::find()->where(['fk_per_municipal'=>$mun_id])->andWhere(['status'=>7])->count();
            $total_initial=\app\models\PaymentChori::find()->where(['fk_municipal'=>$mun_id])->where(['fk_economic_year'=>$year['economic_year']])->Sum('amount');
            $total_deposit=\app\models\OtherMonthPayment::find()->where(['fk_municipal'=>$mun_id])->where(['fk_year'=>$year['economic_year']])->Sum('amount');
                //var_dump($total_initial);die;
                $total_chori=\app\models\ChoriBachat::find()->where(['fk_per_municipal'=>$mun_id])->count();
                $total_initial=\app\models\PaymentChori::find()->where(['fk_municipal'=>$mun_id])->andWhere(['fk_economic_year'=>$year['economic_year']])->Sum('amount');
                $total_deposit=\app\models\OtherMonthPayment::find()->where(['fk_municipal'=>$mun_id])->andWhere(['fk_year'=>$year['economic_year']])->Sum('amount');
            ?>

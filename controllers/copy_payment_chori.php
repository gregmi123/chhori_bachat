...
class PaymentChoriController extends Controller {
    ...
    public function actionInitialDeposit(){
        $user_id = yii::$app->user->id;
        $user_details = \app\models\Users::findOne(['id' => $user_id]);
        $payment_chori=\app\models\PaymentChori::find()->where(['fk_user_id' => $user_id])->andWhere(['fk_municipal' => $user_details->fk_municipal_id])->andWhere(['status' => 6]);
...
//                ->all();
    ...
    public function actionMultiAll($month,$multi_month,$year) {
        ...
        $models = (new \yii\db\Query())
                ->select('chori_bachat.id,chori_bachat.verified_date,chori_bachat.name,chori_account_details.account_open_date,chori_account_details.id as account_id,payment_chori.id as pid,bank_details.id as bank_id,chori_account_details.account_no,bank_details.bank_name')
                ->from('chori_bachat')
                ->join('JOIN', 'chori_account_details', 'chori_account_details.fk_chori_bachat=chori_bachat.id')
                ->join('JOIN', 'bank_details', 'bank_details.id=chori_bachat.fk_bank_details')
                ->join('JOIN', 'payment_chori', 'payment_chori.fk_chori_bachat=chori_bachat.id')
    ...
    public function actionMultiple($month,$multi_month,$year) {
        ...
        //var_dump($final_date);die;
        $account_opened = (new \yii\db\Query())
                ->select('chori_bachat.id,chori_bachat.verified_date,chori_bachat.name,chori_account_details.id as account_id,payment_chori.id as pid,bank_details.id as bank_id,chori_account_details.account_no,bank_details.bank_name,chori_account_details.account_open_date')
                ->from('chori_bachat')
                ->join('JOIN', 'chori_account_details', 'chori_account_details.fk_chori_bachat=chori_bachat.id')
                ->join('JOIN', 'bank_details', 'bank_details.id=chori_bachat.fk_bank_details')
                ->join('JOIN', 'payment_chori', 'payment_chori.fk_chori_bachat=chori_bachat.id')
    ...
    public function actionConfirmAll($month, $year,$multi_month){
        ...
        //var_dump($bank_id->id);die;
        $models = (new \yii\db\Query())
                ->select('chori_bachat.id,chori_bachat.dob,chori_bachat.tole_name,chori_account_details.account_open_date,chori_bachat.father_name,chori_bachat.mobile_no,chori_bachat.created_date,chori_bachat.fk_per_district,chori_bachat.fk_per_municipal,chori_bachat.fk_ward,chori_bachat.verified_date,chori_bachat.name,chori_account_details.id as account_id,payment_chori.id as pid,payment_chori.fk_chori_bachat,bank_details.id as bank_id,chori_account_details.account_no,bank_details.bank_name')
                ->from('chori_bachat')
                ->join(' JOIN', 'chori_account_details', 'chori_account_details.fk_chori_bachat=chori_bachat.id')
                ->join(' JOIN', 'bank_details', 'bank_details.id=chori_bachat.fk_bank_details')
                ->join(' JOIN', 'payment_chori', 'payment_chori.fk_chori_bachat=chori_bachat.id')
    ...
    public function actionConfirm($month, $year,$multi_month){
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $add_month=(int)($model->add_month);
            $fk_month=(int)$model->fk_month;
            $final_date1=array();
        ...
        //var_dump($multipleChhori);die;
            ...
            try {
                ...
                if(((int)$verified_year[0]==(int)$insert_year[0]) && (int)$final_verified_month[0] <=10){  
                //var_dump($multipleChhori);die;
                //var_dump($fk_month);die;
    ...
    public function actionTable(){
        ...
        //var_dump($bank_id->id);die;
        $models = (new \yii\db\Query())
                ->select('chori_bachat.id,chori_bachat.fk_month as month,chori_account_details.account_open_date,chori_account_details.fk_year,chori_bachat.name,chori_account_details.id as account_id,bank_details.id as bank_id,chori_account_details.account_no,bank_details.bank_name')
                ->from('chori_bachat')
                ->join('JOIN', 'chori_account_details', 'chori_account_details.fk_chori_bachat=chori_bachat.id')
                ->join('JOIN', 'bank_details', 'bank_details.id=chori_bachat.fk_bank_details')
    ...
    public function actionReport(){
        $user_id = yii::$app->user->id;
        $user_details = \app\models\Users::findOne(['id' => $user_id]);
        $models=\app\models\PaymentChori::find()->where(['fk_user_id' => $user_id])->andWhere(['fk_municipal' => $user_details->fk_municipal_id])->andWhere(['status' => 6])->all();
    ...
    public function actionCreate() {
        ...
        //var_dump($bank_id->id);die;
        $account_opened = (new \yii\db\Query())
                ->select('chori_bachat.id,chori_bachat.fk_month as month,chori_bachat.name,chori_account_details.id as account_id,bank_details.id as bank_id,chori_account_details.account_no,chori_account_details.account_open_date,chori_account_details.fk_year,bank_details.bank_name')
                ->from('chori_bachat')
                ->join('JOIN', 'chori_account_details', 'chori_account_details.fk_chori_bachat=chori_bachat.id')
                ->join('JOIN', 'bank_details', 'bank_details.id=chori_bachat.fk_bank_details')
                ->where(['chori_bachat.status'=>2])
